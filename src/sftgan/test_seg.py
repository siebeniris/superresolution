...
parser.add_argument('-i', '--input', type=str, help="the file of a list of image paths")
parser.add_argument("-t", '--thirty', action="store_true", default=False,
                    help="if only the 30m images are applied")
args = parser.parse_args()

# options
# test_img_folder_name = args.input.split('_')[0]  # image folder name
test_img_folder_name = args.input

test_img_folder = 'data/interim/datasets/' + test_img_folder_name  # HR images
save_prob_path = 'data/processed/sftgan/' + test_img_folder_name + '/segprob'  # probability maps
save_byteimg_path = 'data/processed/sftgan/' + test_img_folder_name + '/byteimg'  # segmentation annotations
save_colorimg_path = 'data/processed/sftgan/' + test_img_folder_name + '/colorimg'  # segmentaion color results
...
model_path = 'models/sftgan_pretrained_models/segmentation_OST_bic.pth'
